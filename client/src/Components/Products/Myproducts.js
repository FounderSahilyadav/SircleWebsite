...
const Myproducts = ({ studentToken }) => {
        ...
        const [loder, setLoader] = useState(false); // Set loader to true while registering the student
        useEffect(() => {
            // console.log(query.get("course"));
            const set = async() => {
                if (studentToken) {
                    setLoader(true);
                    await fetchCourse(studentToken, setCourses, setError, setSuccess);
                    setLoader(false);
                }
            }
            if (studentToken)
        ...
        return ( <
                div className = 'menu' > {
                    loder ? < CircularProgress className = { classes.circularProgress }
                    /> : < >
                    <
                    h1 >
                    Your Course :
                        ...
                        ( < div >
                            ...
                            div className = "Alert" > {
                                ...
                                div className = { classes.orders } > {
                                    courses.map((course) => {
                                        return < > {
                                            (course.courseName === 'DISHA' || course.courseName === "SAKHA" || course.courseName === "PERSONA") ? ( <
                                ...
                                div className = { classes.orders } > {
                                    courses.map((course) => {
                                        return < > {
                                            (course.courseName === 'DISHA_CRASH' || course.courseName === "ALL_COURSE") ? ( <
